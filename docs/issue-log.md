...
### GitHub Actions test-live 잡과 sync-to-main 잡 충돌 — 동시에 main push 시도

**날짜**: 2026-03-08

**현상**
```
Random-Validation table extracted: No
Auto-merging docs/test-report.md
CONFLICT (content): Merge conflict in docs/test-report.md
Rebasing (1/1)
error: could not apply 8f4bdc2...
```

**원인**
- 두 개의 잡(`test-live`와 `sync-to-main`)이 동시에 `docs/test-report.md`를 수정하고 main에 push를 시도
- `test-live`가 Tier 2 결과를 추가하고 커밋을 시도하는 동시에 `sync-to-main`도 Tier 1 결과를 커밋하려고 함
- 두 잡이 거의 동시에 실행되므로 둘 중 먼저 커밋한 잡에 두 번째 잡이 충돌 발생

**조치사항**
1. `test-live` 잡에서 main에 직접 push하는 부분 제거
2. `test-live`는 Tier 2 결과를 아티팩트로만 저장
3. `sync-to-main` 잡에 `needs: [test-mock, test-live]` 추가하여 두 테스트 모두 완료 후 실행
4. `sync-to-main`에서 Tier 2 아티팩트를 다운로드하여 test-report.md에 병합
5. `sync-to-main`만 main에 커밋 → 충돌 발생 방지
