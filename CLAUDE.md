...
> 1. **테스트 실행 및 레포트 생성** — 모든 푸시 전 반드시 테스트를 실행하고 `docs/test-report.md`를 생성해야 합니다.
>    ```bash
>    npm test
>    ```
>    - Tier 1 테스트가 통과하지 않으면 push 금지
> 2. **`docs/session-context.md`를 반드시 업데이트한다** — 오늘 나눈 대화 내용(결정사항, 변경사항, 다음 할 일)을 빠짐없이 기록.
>    - 사소한 결정도 모두 포함. "왜 그렇게 했는지(Why)"를 꼭 명시할 것.
>    - 세션 이력 테이블에 날짜 + 주요 작업 추가.
> 3. **작업 브랜치에 push** — 필요하면 나중에 수동으로 PR을 생성하거나 main에 머지하세요:
>    ```
>    git push -u origin <작업브랜치>
>    ```
> 4. **session-context.md 업데이트 없이 코드만 커밋하는 것은 금지.**
...
│       ├── testing.md                 # 테스트 스킬
│       └── test-cases-generator.md   # 테스트 케이스 생성 스킬
├── apps-script/
...
│   ├── MpnValidator.gs              # 에이전트13: MPN 스펙 검증
│   ├── OutputFormatter.gs             # 에이전트8: 10열 테이블 출력 포맷팅
...
│   ├── test-mpn-validator.js        # MpnValidator 단위 테스트 (⚠️ mock HTTP)
│   ├── test-integration.js            # 전체 파이프라인 통합 테스트 (mock API)
│   ├── test-mouser-live.js            # Mouser 실제 API 호출 테스트 (Tier 2)
│   ├── test-glm-live.js               # GLM 실제 API 호출 테스트 (Tier 2)
│   ├── test-random-validation.js      # 랜덤 저항값 E2E 검증 + 리포트 생성 (Tier 2)
│   ├── test-cases.json               # 랜덤 저항값 입력 케이스 저장소
│   ├── mocks/
...
│   ├── random-validation-table.md    # 랜덤 검증 테이블
│   ├── test-report.md                 # 테스트 결과 종합 리포트
│   └── user-setup-guide.md            # 사용자 직접 수행 항목 (API 키 발급, 배포 등)
├── .env                             # API 키 저장 (Git 미포함)
└── package.json                       # npm test / npm install
```
...
| `MpnValidator.gs` | ✅ **완전 테스트 가능** | 정규식 기반 추출 로직 | 바로 가능 |
...
OutputFormatter 10열 포맷            │                                      │
ErrorHandler 메시지 생성            │                                      │
Config/Cache/Client DI 로직 (mock)  │                                      │
전체 파이프라인 통합 테스트 (mock)  │                                      │
```

> **Tier 2 구현 방법**: live 테스트 파일(`test-mouser-live.js`, `test-glm-live.js`, `test-random-validation.js`)은 Apps Script의 `UrlFetchApp` 대신 Node.js 내장 `https` 모듈을 직접 사용하여 실제 API 서버에 HTTP 요청을 보낸다. `.env` 파일의 키를 `dotenv`로 로딩해 인증한다.
...
**캐싱**: 동일 입력에 대한 GLM 응답을 CacheManager를 통해 캐싱 (API 비용 절감)
...
3. 추출된 패키지 사이즈를 Metric ↔ Imperial 매핑 테이블 구축
...
### 에이전트 8: OutputFormatter — 10열 테이블 출력
**파일**: `apps-script/OutputFormatter.gs`

**출력 형식 (10열)**:
| 입력 원본 | 추출 저항값 | 추출 패키지 | 추출 오차 | 부품명 (MPN) | Description | MPN 저항값 | MPN 패키지 | MPN 오차 | 일치 확인 |
|-----------|------------|------------|----------|-------------|-------------|------------|-----------|---------|---------|
| 1k 1005 5% | 1kΩ | 0402 (0402) | 5% | RC0402FR-071KL | RES SMD 1K OHM 5% 1/16W 0402 | 1kΩ | 0402 | 5% | ✅ |
| 10k/0603/1% | 10kΩ | 0603 (0603) | 1% | CR0603-FX-1002ELF | RES SMD 10K OHM 1% 1/10W 0603 | 10kΩ | 0603 | 1% | ✅ |

**6-8열 설명**: Mouser API에서 MPN을 기반으로 추출한 실제 저항값/패키지/오차
**9열 설명**: 입력값과 MPN 스펙이 일치하는지 확인 (PASS/FAIL)
...
### 에이전트 11: MpnValidator — MPN 스펙 검증
**파일**: `apps-script/MpnValidator.gs`

StockRanker가 선정한 MPN을 Mouser `SearchByPartNumber` API로 단건 조회하여 Description에서 저항값/패키지/오차를 역추출하고, 입력값과 대조하여 일치 여부를 검증하는 에이전트.

**검증 기준**:
- 저항값: 1% 허용 오차 범위 내 일치
- 패키지: 정확 일치 필요
- 오차: 정확 일치 필요
- 검증 결과: valid (PASS), invalid (FAIL)
...
### 에이전트 13: test-cases.json — 랜덤 저항값 입력 저장소
**파일**: `tests/test-cases.json`

랜덤 저항값 입력 케이스를 저장하는 JSON 파일입니다. 현재는 수동으로 작성된 고정값입니다.
...
**참고**: 현재는 고정된 입력값이 포함되어 있습니다. 실제 랜덤 생성 기능(`/test-cases-generator` 스킬)은 구현되어 있지만 사용하지 않습니다.
...
> .env에 API 키가 있으면 자동으로 Tier 2도 실행됩니다.
...
[MpnValidator]         20/20  ✅
[Integration]          17/17  ✅
[Mouser-Live]          2/2   ✅ (Tier 2)
[GLM-Live]             2/2   ✅ (Tier 2)
[Random-Validation]    12/13  ✅ (Tier 2)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Total: 188/189 passed ✅
```
...
> `docs/test-report.md`는 테스트 실행 결과를 저장합니다.

- 상단: 테스트 일시, 모드, 결과, 통과 메타데이터
- 본문: `## 최종 출력물 (사용자가 실제로 받아보는 결과)` 섹션
- 최종 출력물: 10열 테이블 (입력 원본, 추출 저항값, 추출 패키지, 추출 오차, 부품명(MPN), Description, MPN 저항값, MPN 패키지, MPN 오차, 일치 확인)
- Tier 2 실행 시: 실제 random-validation-table.md 데이터 표시
- Tier 1 실행 시: 예시 데이터 표시
...
- **Tier 2 test-random-validation.js**: `test-cases.json`에서 다양한 입력 케이스 사용
...
> **참고**: Tier 2 Live 테스트(Tier 2) 실제 Mouser/GLM API HTTP 호출 검증은 `npm test` 로 자동 가능 (수동 불필요).
