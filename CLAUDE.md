...
>    - Tier 2 라이브 테스트(`npm run test:live`)는 API 키가 필요할 때만 실행
...
│   ├── MpnValidator.gs              # 에이전트13: MPN 역검증
│   ├── Config.gs                      # API 키 설정
│   └── appsscript.json               # Apps Script 매니페스트
├── blog/
│   └── output.txt                   # 블로그 포스팅용 콘텐츠 가이드 (SEO 최적화 포함)
├── blogger/
...
| `MpnValidator.gs` | ⚠️ **DI + Mock 필요** | `UrlFetchApp` 사용 → mock 응답으로 역검증 로직 테스트 | mock HTTP 필요 |
...
MpnValidator MPN 역검증 로직       │                                      │
OutputFormatter 9열 포맷            │                                      │
ErrorHandler 메시지 생성            │                                      │
...
- 응답에서 저항값, 패키지, 오차 항목들을 추출해서 최종 출력으로 전달
...
### 에이전트 8: OutputFormatter — 9열 테이블 출력
**파일**: `apps-script/OutputFormatter.gs`

**출력 형식 (9열)**:
| 입력 원본 | 추출 저항값 | 추출 패키지 | 추출 오차 | 부품명 (MPN) | Description | MPN 저항값 | MPN 패키지 | MPN 오차 | 일치 확인 |
|-----------|------------|------------|----------|-------------|-------------|------------|-----------|---------|---------|
| 1k 1005 5% | 1kΩ | 0402 (1005) | 5% | RC0402JR-071KL | RES SMD 1K OHM 5% 1/16W 0402 | 1kΩ | 0402 | 5% | ✅ |
| 10k/0603/1% | 10kΩ | 0603 (1608) | 1% | RC0603FR-0710KL | RES SMD 10K OHM 1% 1/10W 0603 | 10kΩ | 0603 | 1% | ✅ |

**6-8열 설명**: Mouser API에서 MPN을 기반으로 추출한 실제 저항값/패키지/오차
**9열 설명**: 입력원본의 저항값/패키지/오차가 MPN의 저항값/패키지/오차와 일치하는지 확인
...
### 에이전트 13: MpnValidator — MPN 역검증
**파일**: `apps-script/MpnValidator.gs`

StockRanker가 선정한 MPN을 Mouser `SearchByPartNumber` API로 단건 조회하여 Description에서 저항값/패키지/오차를 역추출, 입력값과 대조해 일치 여부를 검증하는 에이전트.

**역할**:
- Mouser `POST /api/v2/search/partnumber` 호출
- Description에서 저항값/패키지/오차 역추출
- 입력값과 MPN의 스펙 대조
- 검증 결과 (valid/reason/actual) 반환

**동작 흐름**:
```
StockRanker → MPN 선정 → MpnValidator.validateMpn()
  ├── Mouser Part Number API로 MPN 조회
  ├── Description에서 스펙 역추출
  ├── 저항값/패키지/오차 비교
  └── { valid, reason, actual } 반환
```

**검증 기준**:
- 저항값: 1% 허용 (부동소수점 오차 대비)
- 패키지: 정확 일치 필요
- 오차: 정확 일치 필요

**DI 패턴**: `UrlFetchApp`를 파라미터로 주입하여 테스트 가능
...
> `docs/test-report.md`에는 **사용자가 실제로 받아보는 최종 출력물 표**만 작성합니다.

- 내용: 9열 테이블 예시 (`입력 원본 / 추출 저항값 / 추출 패키지 / 추출 오차 / 부품명(MPN) / Description / MPN 저항값 / MPN 패키지 / MPN 오차 / 일치 확인`)
