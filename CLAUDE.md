...
│   ├── PackageListBuilder.gs          # 에이전트2: Mouser API로 패키지 리스트 추출/정리
│   ├── PackageConverter.gs            # 에이전트3: 패키지 변환 (metric↔imperial)
...
├── tests/
│   ├── run-all-tests.js              # 에이전트10: 전체 테스트 실행 + 피드백 생성
...
PackageConverter 변환 정확도        │  실제 Mouser 키워드 검색 응답        │  Blogger iframe 연동
...
> **Tier 2 구현 방법**: live 테스트 파일(`test-mouser-live.js`)은 Apps Script의 `UrlFetchApp` 대신 Node.js 내장 `https` 모듈을 직접 사용하여 실제 Mouser API 서버에 HTTP 요청을 보낸다. `.env` 파일의 키를 `dotenv`로 로딩해 인증한다.
...
**패키지 목록 소스**: PackageListBuilder 에이전트가 Mouser API를 통해 동적으로 추출한 패키지 리스트를 사용. 하드코딩된 정적 목록 대신 실제 Mouser에서 판매 중인 패키지를 기준으로 매칭.

### 에이전트 2: PackageListBuilder — 패키지 리스트 동적 추출
...
### 에이전트 3: PackageConverter — 패키지 변환
...
- 키: `mouser_{resistance}_{package}_{tolerance}`, `packagelist_...`
- TTL: 1시간 (재고 신선도 유지)
- API rate limit 보호 효과

### 에이전트 8: ErrorHandler — 에러 핸들링 & 로깅
...
> **참고**: 실제 Mouser API HTTP 호출 검증은 `npm run test:live` 로 자동 가능 (수동 불필요)
