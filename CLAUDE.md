...
| 🚫 10 | **사용자에게 노출되는 모든 텍스트(프런트엔드 UI, 백엔드 API 응답 메시지)에 한국어를 사용하는 것 — 블로거 서비스의 모든 사용자 노출 텍스트는 반드시 영어만 사용한다** |
| 🚫 11 | **사용자에게 노출되는 텍스트(UI 라벨, 메시지, SEO 문구, JSON-LD, HTML body)에 "GLM", "glm-4.7", 또는 특정 AI 제조사명을 노출하는 것 — 사용자 노출 텍스트에서는 항상 "AI"로만 표기한다** |
